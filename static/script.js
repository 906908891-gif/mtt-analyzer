...
function holmBonferroni(pValues) {
  ...
  if (n === 0) return [];
  const sorted = pValues.map(function(p, i) { return { p:p, i:i }; }).sort(function(a, b) { return a.p - b.p; });
  const adjusted = new Array(n);
  let cumMin = 1;
  for (let k = 0; k < n; k++) {
    const adj = Math.min(1, sorted[k].p * (n - k));
    cumMin = Math.min(cumMin, adj);
    adjusted[sorted[k].i] = cumMin;
  }
  return adjusted;
}
