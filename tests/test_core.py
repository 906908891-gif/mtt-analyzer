...
def test_fit_perfect_data():
    ...
    assert result is not None, "fit_4pl returned None for 6-point data"
...
def test_fit_too_few_points():
    # 4PL needs at least 4 points; 3 should return None
    result = fit_4pl([-5, -4, -3], [50, 40, 30])
    assert result is None, "expected None for 3-point data"
...
def test_fit_with_noise():
    ...
    result = fit_4pl(log_conc, y)
    assert result is not None
    assert abs(result.EC50 - 10 ** -4.5) / 10 ** -4.5 < 0.5  # noisy LM has wider variance
    assert result.R2 > 0.95


def test_predict_4pl():
    log_conc = [-5, -4, -3, -2]
    y = np.array([80.0, 50.0, 20.0, 5.0])
    result = fit_4pl(log_conc, y)
    assert result is not None
    yh = predict_4pl(log_conc, result.Top, result.Bottom, result.logEC50, result.HillSlope)
    assert np.allclose(yh, y, atol=1.0)
